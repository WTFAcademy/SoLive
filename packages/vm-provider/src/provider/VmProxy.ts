import {  util , helpers , ConsoleLogs, hash } from '@remix-project/remix-lib'
const { toHexPaddedString, formatMemory } = util
const  { normalizeHexAddress } = helpers.ui
import BN from 'bn.js'
import utils, { isBigNumber } from 'web3-utils'
...
export class VmProxy {
  ...
  txsMapBlock
  blocks
  stateCopy: StateManager | undefined
  flagDoNotRecordEVMSteps: boolean | undefined
  lastMemoryUpdate: Array<string> | null | undefined
  ...
  constructor (vmContext: VMContext) {
    ...
    this.sha3Preimages = {}
    // util
    this.sha3 = (...args) => utils.sha3.apply(this, args as any)
    this.toHex = (...args) => utils.toHex.apply(this, args as any)
    this.toAscii = (...args) => utils.toAscii.apply(this, args as any)
    this.fromAscii = (...args) => utils.fromAscii.apply(this, args as any)
    this.fromDecimal = (...args) => utils.fromDecimal.apply(this, args as any)
    this.fromWei = (...args) => utils.fromWei.apply(this, args as any)
    this.toWei = (...args) => utils.toWei.apply(this, args as any)
    this.toBigNumber = (...args) => utils.toBN.apply(this, args as any)
    this.isAddress = (...args) => utils.isAddress.apply(this, args as any)
    this.utils = utils
    this.txsMapBlock = {}
    this.blocks = {}
  ...
  async txWillProcess (data: TypedTransaction) {
    ...
    if (data.to) {
      await (async (processingHash, processingAccount, processingAddress, self: any) => {
        try {
      ...
      if (step.op === 'CREATE' || step.op === 'CALL') {
        if (step.op === 'CREATE') {
          ...
          if (!this.storageCache[this.processingHash][this.processingAddress]) {
            await (async (processingHash, processingAddress, self: any) => {
              try {
                const account = Address.fromString(processingAddress)
