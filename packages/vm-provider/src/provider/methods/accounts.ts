...
export class Web3Accounts {
  ...
  _addAccount (privateKey: any, balance: any) {
    return new Promise((resolve, reject) => {
      ...
      stateManager.getAccount(Address.fromString(addressStr)).then((account: any) => {
        account.balance = new BN(balance.replace('0x', '') || 'f00000000000000001', 16)
        stateManager.putAccount(Address.fromString(addressStr), account).catch((error: unknown) => {
          reject(error)
        }).then(() => {
          resolve({})
        })
  ...
  eth_accounts (_payload: any, cb: any) {
    return cb(null, Object.keys(this.accounts))
  }

  eth_getBalance (payload: any, cb: any) {
    const address = payload.params[0]

    this.vmContext.vm().stateManager.getAccount(Address.fromString(address)).then((account: any) => {
