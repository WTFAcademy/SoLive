...
class VmProvider {
  ...
  constructor({ fork = Hardfork.London }: TVmProviderParams) {
    ...
    this.worker = workCache || (workCache = new Worker(new URL('./worker.js', import.meta.url), { type: 'module' }));
  ...
  async getBalances(addresses: string[]) {
    return Promise.all(addresses.map(async (address) => this.provider.getBalance(address)));
  }
}
